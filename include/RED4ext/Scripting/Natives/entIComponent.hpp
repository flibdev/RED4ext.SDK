...
struct IComponent : IScriptable
{
    ...
    Entity* entity;                             // 50
    IComponentState* replicatedState;           // 58
    CRUID id;                                   // 60
    ResourcePath appearancePath;                // 68
    DynArray<Handle<IAttachment>> attachments;  // 70 entITransformAttachements, etc
    uint16_t transform_offset;                  // 80
    uint16_t unk82;                             // 82
    uint32_t unk84;                             // 84
    Flags flags;                                // 88
    uint8_t unk89;                              // 89
    SharedMutex unk8A;                          // 8A mutex for attachements
    bool isEnabled;                             // 8B
